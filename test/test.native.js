...
var ssum = tryRequire( resolve( __dirname, './../lib/native.js' ) );
var opts = {
	'skip': ( ssum instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof ssum, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', opts, function test( t ) {
	t.strictEqual( ssum.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the sum of all elements in a one-dimensional ndarray', opts, function test( t ) {
	var x;
	var v;

	x = new Float32Array( [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ] );
	v = ssum( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float32Array( [ -4.0, -5.0 ] );
	v = ssum( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, -9.0, 'returns expected value' );

	x = new Float32Array( [ -0.0, 0.0, -0.0 ] );
	v = ssum( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZerof( v ), true, 'returns expected value' );

	x = new Float32Array( [ NaN ] );
	v = ssum( [ vector( x, 1, 1, 0 ) ] );
...
tape( 'if provided an empty ndarray, the function returns `0.0`', opts, function test( t ) {
	var x;
	var v;

	x = new Float32Array( [] );

	v = ssum( [ vector( x, 0, 1, 0 ) ] );
